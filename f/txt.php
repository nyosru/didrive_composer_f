...
function pa($g, $type = null, $type2 = null, $name = null, $show_file_lie_otkuda_vuzov = true) {
    ...
    if ($show_file_lie_otkuda_vuzov === true) {
        $e = debug_backtrace();
...
/**
 * PrintLine печатаем файл и текущую строку
 */
function pl() {
    $e = debug_backtrace();
    //echo '<pre>'; print_r($e); echo '</pre>';
...
function gsm_rus($gsm3, $type = false) {
...
function getTimer($date, $time = null) {
...
// -1 вчера
// -2 позавчера
function checkdates($dat, $types = false) {
...
// дату в текст
function date_in_text($var, $chto) {
...
function txt_okonchanie($cifra, $type = 'ов') {
...
function f_csv($str, $separator = ';', $quote = '"') {
